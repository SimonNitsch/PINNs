...
class PINN:
    ...
    def train(self, l_r, exp, epochs, s_n):
        ...
        opt2 = optimizers.Adam(learning_rate = optimizers.schedules.ExponentialDecay(0.01,1000,0.5))
        ...
        for epoch in range(epochs):
            ...
            loss, grads, factorgrads = self.loss_step(X)
    ...
    def calling(self,xk):
        ...
        if count % 25 == 0:
            ...
            loss, grads, fac = self.loss_step(self.traindict["X"])
    ...
    def loss_step(self,X):
        ...
        with tf.GradientTape(persistent=True) as g:
            ...
            fr = - v_t + self.factors[0][0] * u_xx + self.factors[0][1] * u * (math.square(u) + math.square(v))
            fi = u_t + self.factors[0][2] * v_xx + self.factors[0][3] * v * (math.square(u) + math.square(v))
                

            ypred = self.model(self.xtest)

            upred = tf.cast(ypred[:,0], tf.float64)
            vpred = tf.cast(ypred[:,1], tf.float64)

            utrue = tf.constant(self.utrue, dtype=tf.float64)
            vtrue = tf.constant(self.vtrue, dtype=tf.float64)


            loss = math.reduce_mean(math.square(fr)) + math.reduce_mean(math.square(fi)) + \
                (math.reduce_mean(math.squared_difference(utrue,upred)) + math.reduce_mean(math.squared_difference(vtrue,vpred)))*20
...
if __name__ == "__main__": 
    ...
    # Noise
    utrue += np.random.randn(utrue.shape[0]) * utrue * 0.2
    vtrue += np.random.randn(vtrue.shape[0]) * vtrue * 0.2
