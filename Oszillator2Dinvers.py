...
class PINN:
    ...
    def __init__(self,d_l,ub,lb,t_start,t_end,sigma, n=[[1,0.5,0.3,0],[0.7,0.4,0,0],[0.4,0,0,0],[0,0,0,0]]):
    ...
    def train(self, l_r, exp, epochs, s_n):
        ...
        opt2 = tf.keras.optimizers.Adam(learning_rate = tf.keras.optimizers.schedules.ExponentialDecay(l_r*10,1000,exp*0.76))
        ...
        u_true, v_true = self.solution(x_train,y_train,t_train)
    ...
    @tf.function
    def calculate_loss(self,X,u_true,v_true):
        ...
        with tf.GradientTape(persistent=True) as g:
            ...
            fr = self.factors[0][0] * (u_xx + u_yy) + self.factors[0][1] * (x**2 + y**2) *u/2 + self.h * v_t 
            fi = self.factors[0][2] * (v_xx + v_yy) + self.factors[0][3] * (x**2 + y**2) *v/2 - self.h * u_t 
            ...
            loss = math.reduce_mean(math.square(fr)) + math.reduce_mean(math.square(fi)) + \
                (math.reduce_mean(math.squared_difference(u,u_true)) + math.reduce_mean(math.squared_difference(v,v_true))) * 20
